...
export const Results = (props: { nameProp: string; countryProp: string }) => {
  ...
  const filterAPI = (
    ...
    result.forEach((university: any) => {
      const query =
        type === "name"
          ? university.name.toLowerCase().includes(prop)
          : university.country.toLowerCase().includes(prop);
      ...
      if (props.nameProp || props.countryProp) {
        result.forEach((university: any) => {
          if (
            university.name
              .toLowerCase()
              .includes(props.nameProp.toLowerCase()) &&
            university.country
              .toLowerCase()
              .includes(props.countryProp.toLowerCase())
          ) {
            if (
              !filter.some(
                (data) =>
